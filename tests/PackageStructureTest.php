...
it('has a valid composer.json with correct package name marko/encryption-openssl', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';

    expect(file_exists($composerPath))->toBeTrue()
        ->and(json_decode(file_get_contents($composerPath), true))->toBeArray()
        ->and(json_decode(file_get_contents($composerPath), true)['name'])->toBe('marko/encryption-openssl');
});
...
it('has correct description in composer.json', function () {
    ...
    expect($composer['description'])->toBe('OpenSSL encryption driver for Marko Framework');
});
...
it('requires marko/encryption package', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);

    expect($composer['require'])->toHaveKey('marko/encryption');
});

it('requires ext-openssl extension', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);

    expect($composer['require'])->toHaveKey('ext-openssl');
});

it('has PSR-4 autoloading configured for Marko\Encryption\OpenSsl namespace', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);

    expect($composer)->toHaveKey('autoload')
        ->and($composer['autoload'])->toHaveKey('psr-4')
        ->and($composer['autoload']['psr-4'])->toHaveKey('Marko\\Encryption\\OpenSsl\\')
        ->and($composer['autoload']['psr-4']['Marko\\Encryption\\OpenSsl\\'])->toBe('src/');
});
...
it('has module.php with bindings array', function () {
    ...
    expect($config)->toBeArray()
        ...
        ->and($config['bindings'])->not->toBeEmpty();
});
